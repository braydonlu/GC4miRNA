...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    sidebarLayout(
      ...
      mainPanel(
        h4("Instructions:"),
        p("1. Upload a .fasta file with all miRNA sequences"),
        p("2. Click 'Calculate GC Content'"),
        p("3. The GC contents are generated in sequential order, based on
          what sequences appear first in the FASTA file"),
        br(),
  ...
  tabPanel(
    ...
    sidebarLayout(
      ...
      mainPanel(
        h4("Instructions:"),
        p("1.Input a .csv file containing columns for consensus sequence GC content, 5p 
          sequence GC content, and differential expression"),
        p("2. Press run T-tests"),
        p("3. Find T-test results for both UP and DOWN regulated sequences"),
        p("4. Examine the p-value to determine whether the difference is significant"),
        br(),
  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        fileInput("mirnaListFile", "Upload miRNA List",
                  accept = c("text/plain", ".txt")),
        fileInput("miRDataFile", "Upload miRNA Expression Data",
                  accept = c("text/plain", ".txt")),
        downloadButton("downloadResults", "Download Results")
      ),
      mainPanel(
        h4("Instructions:"),
        p("1. Upload a list of miRNAs in the txt format, separated by new line characters
          (e.g., BLCA_miRNA_list.txt)"),
        p("2. Upload the miRNA Expression Data to be searched through the database
          file containing dbDEMC expression information available in the Github)"),
        p("3. Once uploaded, a tab separated table of generated results should show up"),
        p("4. If you wish, you can download this table by clicking 'Download Results'"),
        br(),
  ...
  tabPanel(
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        p("5. Input these FASTA files into MEME to find motifs."),
        br(),
